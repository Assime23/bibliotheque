...
<select id="utilisateur" name="utilisateur" required>
    ...
    <% if (utilisateurConnecte != null) { %>
        <%-- Afficher l'option pour l'utilisateur connecté uniquement --%>
        <option value="<%= utilisateurConnecte.getNumutilisateur() %>"><%= utilisateurConnecte.getNom() %></option>
    <% } %>
</select><br><br>
...
<!-- Ajouter le script JavaScript pour définir la date d'emprunt par défaut à aujourd'hui -->
<script>
    // Obtenir la date actuelle
    var currentDate = new Date();
    
    // Obtenir la date sous forme de chaîne avec le format "YYYY-MM-DD"
    var formattedDate = currentDate.toISOString().slice(0, 10);
    
    // Définir la valeur de l'élément de formulaire "dateEmprunt" avec la date actuelle
    document.getElementById("dateEmprunt").value = formattedDate;
</script>
